...
export class RecordingService {
  ...
  constructor(private readonly storageService: StorageService, private readonly configService: ConfigService) {
    ...
    this.storageConfig = this.configService.getOrThrow<StorageConfig>(ConfigKey.STORAGE);
  }
  ...
  async getUrl(recording: Recording): Promise<string> {
    ...
    if (this.storageConfig.default === DriverType.LOCAL) {
      ...
      return new URL(join('storage', recording.location), this.appConfig.baseUrl).href;
    }
