...
import { Body, Param, Post, UploadedFile, UseInterceptors } from '@nestjs/common/decorators/index.js';
...
export class PublicController {
  constructor(private readonly orm: MikroORM, private readonly publicService: PublicService) {}
  ...
  @Get('task')
  getNextTask(@Req() req: Request, @PublicStudy() study: Study): Promise<Task | null> {
    return this.orm.em.transactional(async em => {
  ...
  @Post('answer')
  answerQuestion(@Req() req: Request, @Body() dto: AnswerDto, @PublicStudy() study: Study) {
    return this.orm.em.transactional(async em => {
  ...
  uploadFile(@Param('recording') token: string, @UploadedFile() file: Express.Multer.File) {
    return this.orm.em.transactional(async em => {
  ...
  @Post('event')
  createEvent(@Body() dto: CreateEventDto, @Req() req: Request, @PublicStudy() study: Study) {
    return this.orm.em.transactional(async em => {
