...
export class Migration20230326094728 extends Migration {
  async up(): Promise<void> {
    this.addSql('alter table "question" drop constraint if exists "question_type_check";');

    this.addSql('alter table "question" alter column "type" type text using ("type"::text);');

    //  single-line, multi-line, number to text
    this.addSql("update \"question\" set \"type\" = 'text' where \"type\" in ('single-line', 'multi-line', 'number');");

    // convert radio to select
    this.addSql('update "question" set "type" = \'select\' where "type" = \'radio\';');
